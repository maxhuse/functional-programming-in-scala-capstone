...
object Visualization extends VisualizationInterface {
  ...
  //    val lon1 = location1.lon.toRadians
  //    val lat2 = location2.lat.toRadians
  //    val lon2 = location2.lon.toRadians
  //
  //    val deltaAlpha = abs(lon1 - lon2)
  //
  //    val centralAngle = atan(
  //      sqrt(pow(cos(lat2) * sin(deltaAlpha), 2) + pow(cos(lat1) * sin(lat2) - sin(lat1) * cos(lat2) * cos(deltaAlpha), 2)) /
  //        (sin(lat1) * sin(lat2) + cos(lat1) * cos(lat2) * cos(deltaAlpha))
  //    )
